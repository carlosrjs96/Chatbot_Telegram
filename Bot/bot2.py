...
def personal(update: Update, context: CallbackContext) -> int:
    ...
    if context.user_data:
        ...
        reply_list.extend([f'->>>> {key} / {str(value_pair)}' for (key, value_pair) in context.user_data.items()])
    else:
...
updater = Updater("5133692731:AAEnzvIWcMtIhvzPGMeSXYI6ej_Z_xcd0C0", persistence=PicklePersistence(filename='bot_data'))
